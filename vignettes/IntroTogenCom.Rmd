---
title: "Introduction to genCom"
author: "Olivia Sabik"
date: "`r Sys.Date()`"
output:
  BiocStyle::html_document
vignette: >
  %\\VignetteIndexEntry{Introduction to genCom}
...
# Mark3 eQTL and BMD GWAS: A Demonstration of gencom 
...
GenCom will accept any association dataset, as long as it contains a column with chromosome information, a column with chromosome position, and a column with summary statistics, either p-values or -log10(p-values). 
...
# Formatting input association data using GenCom

The first step for using GenCom is to format your input data. The gencom_format.R function will unify the column names in your data so they are compatible with GenCom and with one another. This function takes your input dataframe, and the index of the columns containing information about the chromosome, location, and p-value or -log10(p-value). 

```{r, message=FALSE, warning=FALSE}
head(mirrorplot::gencom_format(assoc_data = mark3_bmd_gwas, chr_col = 3, pos_col = 4, p_col = 11))
```

As you can see, the output of GenCom format looks quite a bit like the input, but there are a few subtle changes. Some of the column names have been changed for compatibility, and a -log10(p-value) column had been calculated and named LOG10P. You do need to save this new object to be able to use it in the plotting function. 

```{r message=FALSE, warning=FALSE}
mark3_bmd_gwas_f = mirrorplot::gencom_format(assoc_data = mark3_bmd_gwas, chr_col = 3, pos_col = 4, p_col = 11)
mark3_eqtl_f = mirrorplot::gencom_format(assoc_data = mark3_eqtl, chr_col = 10, pos_col = 11, p_col = 7)
```

# Adding linkage disequilibrium information with genCom
...
```{r warning=FALSE}
head(mirrorplot::gencom_ld(assoc_data = mark3_bmd_gwas_f, rs_col = 2, pops = "EUR", lead_snp = "rs11623869"))
```

As with genCom_format(), the output of genCom_ld() is a modified data frame, now with an LD column and an LD_BIN column, which will be used in the plot. These will need to be saved as well.

```{r warning=FALSE}
mark3_bmd_gwas_f_ld = mirrorplot::gencom_ld(assoc_data = mark3_bmd_gwas_f, rs_col = 2, pops = "EUR", lead_snp = "rs11623869")
mark3_eqtl_f_ld = mirrorplot::gencom_ld(assoc_data = mark3_eqtl_f, rs_col = 15, pops = "EUR", lead_snp = "rs11623869")
```

# Plotting a single dataset with genCom

Now that we have our data formatted and complete, we can plot our associations. First, we will plot each association separately, using the single_plot_function() function. The single_plot_function() requires your formatted association dataframe, with optional ld formatting if you would like to include ld information, the chromosome you would like to plot, the build of the genome you would like to use to plot the genes (default = hg19), and the method you wish to plot by. You can plot the association by:
...
```{r warning=FALSE}
mirrorplot::single_plot_function(assoc_data = mark3_bmd_gwas_f_ld, chr = 14, build = "hg19", plotby = "gene", gene_plot = "MARK3")
```

Example 2: Plotting the GWAS association around the lead snp, rs11623869
```{r warning=FALSE}
mirrorplot::single_plot_function(assoc_data = mark3_bmd_gwas_f_ld, chr = 14, build = "hg19", plotby = "snp", snp_plot = "rs11623869")
```

Example 3: Plotting the eQTL by specifying coordinates
```{r warning=FALSE}
mirrorplot::single_plot_function(assoc_data = mark3_eqtl_f_ld, chr = 14, build = "hg19", plotby = "coord", start_plot = 103500000, end_plot = 104500000)
```

# Making a mirrorplot with genCom

Finally, genCom can help you to compare the assocations by making what we call a mirrorplot. The function takes two formatted association data frames, and returns a plot with dataset #1 on the bottom, and dataset #2 inverted on the top. This allows for a more direct visual comparsion of the two datasets. 

```{r warning=FALSE}
mirror_plot_function(assoc_data1 = mark3_bmd_gwas_f_ld, assoc_data2 = mark3_eqtl_f_ld, chr = 14, plotby = "gene", gene_plot = "MARK3")
```



# Related References:
- coloc package (https://cran.r-project.org/web/packages/coloc/index.html)
